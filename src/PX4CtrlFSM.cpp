...
void PX4CtrlFSM::process() {
  ...
  switch (state) {
    case MANUAL_CTRL: {
      if (rc_data.enter_hover_mode)  // Try to jump to AUTO_HOVER
      {
        ...
        if (cmd_is_received(now_time)) {
          ROS_ERROR(
              "[px4ctrl] Reject AUTO_HOVER(L2). You are sending commands before "
              "toggling into AUTO_HOVER, which is not allowed. Stop sending commands "
              "now!");
          break;
        }
                         ...
                         TAKEOFF)  // Try to jump to AUTO_TAKEOFF
      {
        ...
        if (odom_data.v.norm() > 0.1) {
          ROS_ERROR(
              "[px4ctrl] Reject AUTO_TAKEOFF. Odom_Vel=%fm/s, non-static takeoff is "
              "not allowed!",
              odom_data.v.norm());
          break;
        }
      ...
      if (rc_data
              .toggle_reboot)  // Try to reboot. EKF2 based PX4 FCU requires reboot when its state estimator goes wrong.
      {
    ...
    case AUTO_HOVER: {
      ...
      } else if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd ==
    ...
    case AUTO_TAKEOFF: {
      if ((now_time - takeoff_land.toggle_takeoff_land_time).toSec() <
          AutoTakeoffLand_t::
              MOTORS_SPEEDUP_TIME)  // Wait for several seconds to warn prople.
      {
    ...
    case AUTO_LAND: {
      if (!rc_data.is_hover_mode || !odom_is_received(now_time)) {
        ...
        if (print_once_flag) {
          ROS_INFO(
              "\033[32m[px4ctrl] Wait for abount 10s to let the drone arm.\033[32m");
...
void PX4CtrlFSM::land_detector(const State_t state,
...
Desired_State_t PX4CtrlFSM::get_rotor_speed_up_des(const ros::Time now) {
  ...
  double des_a_z =
      ...
      7.0;  // Parameters 6.0 and 7.0 are just heuristic values which result in a saticfactory curve.
  if (des_a_z > 0.1) {
